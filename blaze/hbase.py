import happybase
import happybase_mock
from toolz.curried import take, merge, map, pipe, second, concat, pluck
from cytoolz import get
from collections import Iterator

from .dispatch import dispatch
from datashape import var, Record, string

def remove_column_families(d):
    """

    >>> remove_column_families({'cf1:foo': 1, 'cf2:bar': 2})
    {'bar': 1, 'foo': 2}
    """
    return dict((key.split(':')[-1], value) for key, value in d.items())


hbase = (happybase.Table, happybase_mock.Table)

@dispatch((happybase.Table, happybase_mock.Table))
def discover(t, nrows=50):
    data = list(take(nrows, t.scan(batch_size=nrows)))

    names = pipe(data, pluck(1), concat, set, sorted)
    clean_names = [name.split(':')[-1] for name in names]

    data = pipe(data, pluck(1), pluck(names), list)

    tuple_type = discover(data).subshape[0][0]
    record_type = Record(list(zip(['row'] + list(clean_names),
                                  [string] + list(tuple_type.dshapes))))

    return var * record_type


@dispatch(Iterator, hbase)
def into(a, b, names=None, chunksize=100, **kwargs):
    if not names:
        nrows = 20
        data = list(take(nrows, b.scan(batch_size=nrows)))
        names = pipe(data, map(second),
                           concat, set, sorted)

    seq = b.scan(batch_size=chunksize)

    return ((row,) + get(names, d, default=None)
            for row, d in b.scan(batch_size=chunksize))


@dispatch((list, tuple, set), hbase)
def into(a, b, **kwargs):
    if not isinstance(a, type):
        a = type(a)
    return a(into(Iterator, b, **kwargs))
